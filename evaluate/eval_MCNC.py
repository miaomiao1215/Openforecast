...
parser.add_argument("--test_category", type=str, default='MCNC', help="do predict")
parser.add_argument("--dataset", type=str, default='../dataset/test.json', help="do predict")

args = parser.parse_args()

prompt = "Based on the given event background, please select one option from the four candidate event options that is most likely to occur in the future. Your final answer should be a single option letter, in the form (option letter) such as (A), at the end of your response. \n Event background: \n{background} \n Event options: \n{options}\n Answer: "
...
if 'falcon' in model_path:
    llm = LLM(model=model_path, tensor_parallel_size=4, quantization=None, dtype='bfloat16')
else:
    llm = LLM(model=model_path, tensor_parallel_size=4, quantization=None, dtype='bfloat16', trust_remote_code=True)
