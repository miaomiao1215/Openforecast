...
.. sourcecode:: pycon
    ...
    >>>
    >>>         # each article has a "title", a mapping of category and a list of "section_titles" and
    >>>         # "section_texts".
...
def segment_all_articles(file_path, min_article_character=200, workers=None, include_category=False):
    ...
    include_category: bool
        Whether or not category should be included in the output
...
def segment_and_write_all_articles(file_path, output_file, min_article_character=200, workers=None,
    ...
    The output format is one article per line, in json-line format with 4 fields::
        ...
        (Optional) 'section_category' - list of category in the article.
...
def segment(page_xml, include_category=False):
    ...
    include_category : bool
        Whether or not category should be parsed.
    ...
    if include_category:
        return title, sections, category
    else:
...
class _WikiSectionsCorpus(WikiCorpus):
    ...
    def __init__(self, fileobj, min_article_character=200, processes=None,
                 lemmatize=None, filter_namespaces=('0',), include_category=False):
        """
        Parameters
        ----------
        fileobj : file
    ...
    def get_texts_with_sections(self):
        ...
        f_w = open('skipped_length.txt', 'w')
        f_w_redirect = open('skipped_redirect.txt', 'w')
        for group in utils.chunkize(page_xmls, chunksize=10 * self.processes, maxsize=1):
            for data in group:
                segment(data, include_category=self.include_category)
            for article in pool.imap(partial(segment, include_category=self.include_category),
                ...
                if not sections or sections[0][1].lstrip().lower().startswith("#redirect"):  # filter redirect
                    skipped_redirect += 1
                    # print(article_title)
                    f_w_redirect.writelines(json.dumps({'title': article_title, 'sections': sections}, ensure_ascii=False) +'\n')
                    continue
                if sum(len(body.strip()) for (_, body) in sections) < self.min_article_character:
                    # filter stubs (incomplete, very short articles)
                    skipped_length += 1
                    f_w.writelines(json.dumps({'title': article_title, 'sections': sections}, ensure_ascii=False) +'\n')
                    # print(article_title)
                    continue
                total_articles += 1
                ...
                if self.include_category:
                    category = article[2]
                    yield (article_title, sections, category)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-i', '--include-category',
        help='Include a mapping for category to other articles in the dump. The mappings format is: '
             '"category": [("article_title_1", "interlink_text_1"), ("article_title_2", "interlink_text_2"), ...]',
        action='store_true'
    )
